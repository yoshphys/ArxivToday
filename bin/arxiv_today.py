...
class MyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if self.dodump: print("Encountered a start tag:", tag)
        self.flag[tag] = True
    ...
    def handle_endtag(self, tag):
        if self.dodump: print("Encountered an end tag :", tag)
    ...
    def handle_data(self, data):
        if self.dodump: print("Encountered some data  :", data)
        if not ("entry" in self.flag.keys() and self.flag["entry"]): return
        ...
        if "id" == self.tag and self.flag[self.tag]:
            ...
            self.entry[self.tag] = html.unescape(re.sub(r" {2,}", ' ', data.replace('\n', ' ').strip()))
        else:
            pass
...
def main():
    ...
    for key, entry in entries.items():
        ...
        if n < MaxTranslations:
            nc += inc
            body += "- transliation: " + translate(deeplurl, params, entry["summary"]) + '\n'
        body += "- link: " + entry["id"] + '\n'
        body += "\n\n"
